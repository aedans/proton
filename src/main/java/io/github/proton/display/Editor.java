...
public final class Editor<T> {
  ...
  public static <T> Option<Projection.Char<T>> character(
      Projection<T> projection, TerminalPosition selected) {
    return projection.characters().get(selected);
  }

  public static <T> Option<TerminalPosition> left(
      Projection<T> projection, TerminalPosition cursor) {
  ...
  public static <T> Option<TerminalPosition> right(
      Projection<T> projection, TerminalPosition cursor) {
  ...
  public Editor<T> update(KeyStroke keyStroke) {
    ...
    switch (keyStroke.getKeyType()) {
      ...
      case ArrowUp:
        return new Editor<>(
            style, projector, tree, cursor.withRow(Math.max(0, cursor.getRow() - 1)));
      case ArrowDown:
        return new Editor<>(
            style,
            projector,
            tree,
            cursor.withRow(Math.min(projection.rows() - 1, cursor.getRow() + 1)));
      case Character:
            ...
            .map(
                character -> {
                  ...
                  return new Editor<>(
                      ...
                      projector,
                      t2,
                      right(projector.project(t2), selected).getOrElse(cursor));
                })
            .getOrElse(this);
      case Backspace:
            ...
            .map(
                character -> {
                  ...
                  return new Editor<>(
                      ...
                      projector,
                      t2,
                      left(projector.project(t2), selected).getOrElse(cursor));
                })
            .getOrElse(this);
      case Enter:
        return character(projection, selected)
