...
public final class FileLinkProjector implements Projector<FileLink> {
    ...
    public Projection<FileLink> project(FileLink fileLink) {
        Projection<FileLink> name = () -> new LineProjector()
                .project(new Line(fileLink.file.getName()))
                .characters()
                .mapValues(
                        c -> c.map(x -> fileLink).onSubmit(new FileLink(fileLink.tree, fileLink.file, !fileLink.open)));
        if (fileLink.open) {
            Projector<Object> projector = Projector.get((Class) fileLink.tree.getClass());
            Projection<FileLink> content = () ->
                    projector.project(fileLink.tree).characters().mapValues(c ->
                            c.map(object -> new FileLink(object, fileLink.file, true)));
