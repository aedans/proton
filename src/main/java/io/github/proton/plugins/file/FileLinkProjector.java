...
public final class FileLinkProjector implements Projector<FileLink> {
    ...
    public Projection<FileLink> project(FileLink fileLink) {
        Projection<FileLink> name = () -> new LineProjector().project(new Line(fileLink.file.getName())).characters()
                .mapValues(c -> c.map(x -> fileLink)
        ...
        if (fileLink.open) {
            ...
            Projection<FileLink> content = () -> projector.project(fileLink.tree).characters()
