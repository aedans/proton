...
public final class JavaClassDeclarationProjector implements Projector<JavaClassDeclaration> {
  ...
  public Projection<JavaClassDeclaration> project(JavaClassDeclaration classDeclaration) {
    Projection<JavaClassDeclaration> label =
        new LabelProjection("class ", "keyword").map(x -> classDeclaration);
    Projection<JavaClassDeclaration> projection =
        Projector.get(Line.class)
            .project(classDeclaration.name)
            .map(name -> new JavaClassDeclaration(name, classDeclaration.fields));
    Projection<JavaClassDeclaration> openBracket =
        LabelProjection.openBracket.map(x -> classDeclaration);
    Projection<JavaClassDeclaration> closeBracket =
        LabelProjection.closeBracket.map(x -> classDeclaration);
    Projection<JavaClassDeclaration> fields =
        new GroupProjection<JavaClassDeclaration, JavaFieldMember>() {
          @Override
