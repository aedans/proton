...
public final class JavaImportDeclarationProjector implements Projector<JavaImportDeclaration> {
    ...
    public Projection<JavaImportDeclaration> project(JavaImportDeclaration importDeclaration) {
        Projection<JavaImportDeclaration> label = new LabelProjection("import", "keyword")
                .map(x -> importDeclaration);
        Projection<JavaImportDeclaration> projection = Projector.get(Line.class).project(importDeclaration.name)
